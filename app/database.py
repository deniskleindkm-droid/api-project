...
def create_db():
    ...
    with engine.connect() as conn:
        ...
        conn.execute(text("ALTER TABLE product ADD COLUMN IF NOT EXISTS silverbene_cost float"))
        conn.execute(text("ALTER TABLE product ADD COLUMN IF NOT EXISTS markup_used float"))
        conn.execute(text("ALTER TABLE product ADD COLUMN IF NOT EXISTS shipping_cost float"))
        ...
        conn.execute(text("ALTER TABLE product ADD COLUMN IF NOT EXISTS content_generated_at timestamp"))
        conn.execute(text("ALTER TABLE collection ADD COLUMN IF NOT EXISTS video_url varchar(500)"))
